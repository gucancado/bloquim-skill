...
⚠️ **Bug de backend conhecido:** `set_task_schedule` (ou `update_task` com `scheduleMode`+datas) num **card de plano** retorna **HTTP 500** e deixa estado parcial (não há mecanismo de deleção confirmado no código — trate como bug aberto, não como perda garantida). Em card de plano, hoje só ajuste `priority`/`title`/`description` via `update_task`. Pra agendar, crie a task standalone-de-workspace com o prazo e use `attach_task_to_plan`.
