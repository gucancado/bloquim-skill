...
Card de plano nasce `medium`/`sem_prazo`/`createdBy:null`, mas depois é uma task normal: `set_task_schedule` e `update_task` (priority/scheduleMode/datas/título) funcionam direto no card (verificado ok). A criação em modo plano só **rejeita** esses campos no `create_task` — ajuste após criar. Alternativa quando precisar controlar o assignee: crie a task standalone-de-workspace já configurada e use `attach_task_to_plan`.
